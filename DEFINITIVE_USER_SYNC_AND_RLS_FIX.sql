-- Step 1: Safely update the foreign key constraint to allow for ID synchronization.
-- This temporarily removes the existing rule, allows the update to happen, and then recreates
-- the rule with ON UPDATE CASCADE, which is the best practice to prevent this issue in the future.
...
-- Re-create the foreign key constraint with ON UPDATE CASCADE.
...
-- Step 3: Clean up the incorrect user_profiles table and triggers that were created by mistake.
DROP TABLE IF EXISTS public.user_profiles;
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Step 4: Create a trigger to automatically sync new users with your existing public.users table.
...
-- Step 5: The corrected function to get user role (this will now work).
...
-- Step 6: Re-apply the corrected RLS policies for the equipment table.
