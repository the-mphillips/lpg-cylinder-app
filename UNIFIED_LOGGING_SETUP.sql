-- =====================================================
-- UNIFIED AUDIT LOGGING SYSTEM FOR LPG CYLINDER APP
-- =====================================================
...
-- Step 6: Migration from existing tables (optional - run if you want to preserve existing data)
...
SELECT 
    created_at,
    'system',
    UPPER(level::text),  -- Cast enum to text first, then apply UPPER
    'system_event',
    message,
    module,
    TRUE
...
-- Migrate activity_logs (joining with users for email/name)
INSERT INTO audit_logs (
...
SELECT 
    ...
    'INFO',
    al.action,
    COALESCE(al.action, 'User activity'),
    al.user_id,
    u.email,
    ...
    al.resource_id,
    CASE 
        WHEN al.details IS NOT NULL THEN al.details::jsonb
        ELSE NULL
    END,
    FALSE
...
SELECT 
    ...
    END,
    'email_' || status,
    'Email ' || status || ' to ' || recipient_email,
    'email',
    jsonb_build_object(
...
WHERE NOT EXISTS (
    SELECT 1 FROM audit_logs al 
    WHERE al.created_at = email_logs.created_at 
    AND al.details->>'recipient' = email_logs.recipient_email
);
